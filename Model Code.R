...
#################Scenarios for curve fitting paper

scen <- read.csv('./params curve fitting scenarios.csv')
...
seir1 <- function(t, x, parms) {
  ...
  with(as.list(c(parms, x)), {
    ...
    #ef <- ifelse(t < t2, 0, ifelse( t< t3, 0.45, mag))
    ef <- ifelse(t < t2, 0, ifelse( t< t3, mag, ifelse(t<t4, mag1, ifelse(t<t5, mag2, ifelse (t<t6, mag3, mag4)))))
    ##ef <- ifelse (t< t2, 0, mag)
    maska <- ifelse(t< 73, 0, maska)
    siI <- ifelse (t < t1, 0, siI)
    #I <- ifelse(t<t0, 0, 1)
    
    dS  <-    - (I+I2+I3)*(beta*(1-(maska*0.03))*lambda*S*(1-siI)*(1-ef))/N - (beta*(1-(maska*0.2667))*S*(A+A2+A3)*(1-ef))/N 
    dE  <-    - E/alpha   + (I+I2+I3)*(beta*(1-(maska*0.03))*lambda*S*(1-siI)*(1-ef))/N + (beta*(1-(maska*0.2667))*S*(A+A2+A3)*(1-ef))/N 
    dI  <- (E*pS)/alpha - I*(gam) 
    ...
    dS2  <-    - (I+I2+I3)*(beta*(1-(maska*0.03))*lambda*S2*(1-siI)*(1-ef))/N - (beta*(1-(maska*0.2667))*S2*(A+A2+A3)*(1-ef))/N 
    dE2  <-    - E2/alpha   + (I+I2+I3)*(beta*(1-(maska*0.03))*lambda*S2*(1-siI)*(1-ef))/N + (beta*(1-(maska*0.2667))*S2*(A+A2+A3)*(1-ef))/N 
    ...
    dS3  <-    - (I+I2+I3)*(beta*(1-(maska*0.03))*lambda*S3*(1-siI)*(1-ef))/N - (beta*(1-(maska*0.2667))*S3*(A+A2+A3)*(1-ef))/N 
    dE3  <-    - E3/alpha   + (I+I2+I3)*(beta*(1-(maska*0.03))*lambda*S3*(1-siI)*(1-ef))/N + (beta*(1-(maska*0.2667))*S3*(A+A2+A3)*(1-ef))/N 
    ...
    list(der,Iht = Ih + Ih2 + Ih3 + Ic + Ic2 + Ic3,
         Ict = Ic + Ic2 + Ic3)
  })
}
...
#library(dplyr)
all <-  as.data.frame(cbind(rep(1:23, each=501), do.call("rbind", covid_ts)))
all$scenario <- all$V1
all$V1 <- NULL

all.scen <- merge(scen, all, by = "scenario")

write.csv(all.scen, './allscenarios-cfpaper.csv', row.names = F)
